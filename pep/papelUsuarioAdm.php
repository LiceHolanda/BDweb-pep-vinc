...
<body>
    ...
    <div class="box-search">
        <input type="search" class="form-control w-25" placeholder="Buscar papéis cadastrados no PEP" id="pesquisar">
...
<p style= "text-align: center; color: rgba(0,0,0,0.6); font-size: 13px; "> (Filtrar)
...
{
    $data = $_GET['search'];
    $sql = "SELECT * FROM usuario_papel WHERE papel_id LIKE '%data%' ORDER BY id DESC";
}

else
{
    $sql = "SELECT * FROM usuario_papel ORDER BY id DESC";
}
$result = $conn->query($sql);
echo "<p><br><Funções cadastrados no sistema:</u></p>";
...
<br>
    ...
    function searchData()
    {
        window.location = 'papelUsuarioAdm.php?search='+search.value;
    }
        </script>
    </html>